 ...
 */
export async function airdropToken(tokenType: TokenType, amount: number, recipientTA: PublicKey, cluster: Cluster = 'devnet', wardenURL: string = getDefaultWarden(cluster)): Promise<ConfirmedSignatureInfo> {
    ...
    const communicator = new WardenCommunicator(wardenURL);
